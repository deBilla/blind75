...
export default function PythonTopicPage() {
  const { id } = useParams()
  const t = pythonTopics[id]

  if (!t) return <div style={{ padding: '2rem', color: '#64748b' }}>Topic not found</div>

  const cat = pythonCategories.find(c => c.id === t.category)
    ...
    <main className="problem-page fade-in">
      ...
      <div className="problem-page-header">
        ...
        <div className="problem-meta">
          ...
          {cat && (
            <span className="complexity-pill" style={{ color: cat.color }}>
              {cat.name}
            </span>
          )}
        </div>
      ...
      {/* Sections — each has a title, explanation, and code */}
      {t.sections && t.sections.map((sec, idx) => (
        <div key={idx}>
          ...
          {sec.code && (
            <CodeBlock code={sec.code} lang="Python" />
          )}
        </div>
      ))}
