...
export default function LangCategoryPage({ categories, topics, categoryTopics, topicBasePath }) {
  const { id } = useParams()
  const cat = categories.find(c => c.id === id)
  const topicIds = categoryTopics[id] || []
  ...
  return (
    <main className="category-page">
      <div className="cat-header" style={{ borderColor: cat.color + '44', background: cat.bg }}>
        <div style={{ color: cat.color, fontSize: '1.4rem', fontFamily: 'JetBrains Mono', fontWeight: 700, marginBottom: 8 }}>
          {cat.icon}
        </div>
      ...
      <div className="problems-grid">
        {topicIds.map(tid => {
          ...
          return (
            <Link key={tid} to={`${topicBasePath}/${tid}`} className="problem-card" style={{ '--card-color': cat.color }}>
              ...
              <span style={{
                ...
                fontSize: '0.72rem', padding: '3px 10px', borderRadius: 20, whiteSpace: 'nowrap', fontWeight: 500,
              }}>
                {t.level}
              </span>
            </Link>
